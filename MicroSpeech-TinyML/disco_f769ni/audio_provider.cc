...
TfLiteStatus InitAudioRecording(tflite::ErrorReporter* error_reporter) 
{
  ...
  if (BSP_AUDIO_IN_Init(BSP_AUDIO_FREQUENCY_16K,
                        DEFAULT_AUDIO_IN_BIT_RESOLUTION,
                        DEFAULT_AUDIO_IN_CHANNEL_NBR) == AUDIO_ERROR) {
    TF_LITE_REPORT_ERROR(error_reporter , "BSP_AUDIO_IN_Init error");
  }

  /* Allocate scratch buffers */
  if (BSP_AUDIO_IN_AllocScratch(Scratch, SCRATCH_BUFF_SIZE) == AUDIO_ERROR) {
    TF_LITE_REPORT_ERROR(error_reporter , "BSP_AUDIO_IN_AllocScratch error\n");
  }

  /* Start Recording */
  if (BSP_AUDIO_IN_Record((uint16_t*)AUDIO_BUFFER_IN, AUDIO_BLOCK_SIZE) == AUDIO_ERROR) {
    TF_LITE_REPORT_ERROR(error_reporter , "BSP_AUDIO_IN_Record error");
  }

  if (BSP_AUDIO_OUT_Init(OUTPUT_DEVICE_HEADPHONE, 70,
                         BSP_AUDIO_FREQUENCY_16K) == AUDIO_ERROR) {
    TF_LITE_REPORT_ERROR(error_reporter, "BSP_AUDIO_OUT_Init error");
  }

  // Also play results out to headphone jack.
  if (BSP_AUDIO_IN_GetChannelNumber() > 2) {
  ...
  if (BSP_AUDIO_OUT_Play((uint16_t*)AUDIO_BUFFER_OUT, AUDIO_BLOCK_SIZE * 2) == AUDIO_ERROR) {
    TF_LITE_REPORT_ERROR(error_reporter , "BSP_AUDIO_OUT_Play error");
  }
...
TfLiteStatus GetAudioSamples(tflite::ErrorReporter* error_reporter,
  ...
  if (!g_is_audio_initialized) {
    ...
    if (init_status != kTfLiteOk) {
      TF_LITE_REPORT_ERROR(error_reporter, "InitAudioRecording failed : %d", init_status);
      return init_status;
    }
